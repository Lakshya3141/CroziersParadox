...
int main() {
    ...
    // Create a new nest with initial mean cues
    Nest initialNest2(nestID, p);
    std::cout << "Initial Nest ID: " << initialNest2.nest_id << std::endl;
    std::cout << "Initial Nest Mean Cues: ";
    for (auto cue : initialNest2.NestMean) {
        std::cout << cue << " ";
    }
    std::cout << std::endl;
    ...
    // Print individual cues in the initial nest
    std::cout << "Individual Cues in Initial Nest:" << std::endl;
    for (const auto& worker : initialNest2.NestWorkers) {
        std::cout << "Individual ID: " << worker.nest_id << worker.ind_id << " Cues: ";
        for (auto cue : worker.IndiCues) {
            std::cout << cue << " ";
        }
    ...
    std::vector<double> prevNestMean = initialNest.NestMean;
