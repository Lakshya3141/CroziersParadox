...
model_cue_diversity_metabolic <- lm(bcnest_avg ~ sMi * dMetabolicCost, data = filtered_data)
model_cue_abundance_metabolic <- lm(cueabun_avg ~ sMi * dMetabolicCost, data = filtered_data)

summary(model_cue_diversity_metabolic)
summary(model_cue_abundance_metabolic)

# Save summaries of the models to CSV files
write.csv(summary(model_cue_diversity_metabolic)$coefficients, 
          file = paste0("./additional/", name_file, "/model_cue_diversity_metabolic_summary.csv"), 
          row.names = TRUE)

write.csv(summary(model_cue_abundance_metabolic)$coefficients, 
          file = paste0("./additional/", name_file, "/model_cue_abundance_metabolic_summary.csv"), 
          row.names = TRUE)

# # Perform pairwise comparisons for Cue Diversity model
# pairwise_cue_diversity <- emmeans(model_cue_diversity_metabolic, pairwise ~ sMi | dTickTime)
...
# pairwise_cue_abundance <- emmeans(model_cue_abundance_metabolic, pairwise ~ sMi | dTickTime)
