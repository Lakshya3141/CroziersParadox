...
num_samples <- 20  # Specify the number of samples
...
correct_dExpParam <- function(sim_data) {
  ...
  sim_data$modelnew <- paste0(sim_data$iModelChoice,"_",sim_data$iKillChoice)
  sim_data$ltime <- sim_data$gtime/10000
  sim_data <- sim_data %>%
...
filtered_data <- correct_dExpParam(filtered_data)
